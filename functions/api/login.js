/**
 * POST /api/login   — autentica al usuario y devuelve un JWT
...
export async function onRequestPost(context) {
  ...
  if (!email || !password) return jsonRes(fail('Email y contraseña son requeridos.'), 400);
  ...
  try {
    ...
    console.error('[Login] Error leyendo usuario Firebase:', err.message);
    return jsonRes(fail('Error conectando con la base de datos: ' + err.message, 'DB_ERROR'), 503);
  }
  ...
  if (legacy) return jsonRes(fail('Cuenta creada en el servidor anterior. Por favor regístrate de nuevo.', 'LEGACY_ACCOUNT'), 401);
  if (!pwOk) {
    await fbUpdate({
  ...
  if (control.accountStatus !== 'active') return jsonRes(fail('Tu cuenta no está activa.', 'INACTIVE'), 403);
  if (control.permissions?.canLogin === false) return jsonRes(fail('No tienes permiso para iniciar sesión.', 'NO_LOGIN_PERMISSION'), 403);
  ...
  // Best-effort: asegurar que también existe en Firebase Authentication.
  ...
  const token = await signJwt({ uid, username: user.username, email: user.email }, env.JWT_SECRET, env.JWT_EXPIRES_IN || '7d');
  return jsonRes({
    success: true,
